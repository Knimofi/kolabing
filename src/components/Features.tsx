...
const FeatureCard = ({ icon, title, description, index }: FeatureCardProps) => {
  ...
  return (
    <div 
      ref={cardRef}
      className={cn(
        "feature-card opacity-0 text-card-foreground",
        "hover:scale-[1.02] group cursor-pointer"
      )}
      style={{ animationDelay: `${0.1 * index}s` }}
    >
      <div className="rounded-xl bg-primary/10 w-14 h-14 flex items-center justify-center text-primary mb-6 group-hover:bg-primary group-hover:text-primary-foreground transition-all duration-300">
        {icon}
      </div>
...
const Features = () => {
  ...
  return (
    <section className="py-20 md:py-32 bg-background" id="features" ref={sectionRef}>
      <div className="section-container">
        <div className="text-center mb-20">
          <div className="inline-flex items-center px-6 py-3 rounded-full bg-primary/10 border border-primary/20 text-primary mb-8 opacity-0 fade-in-element">
            <span className="text-sm font-bold">Features</span>
          </div>
          <h2 className="section-title mb-8 opacity-0 fade-in-element">
            Advanced Intelligence, <br />
            <span className="text-primary">Human-Like Intuition</span>
          </h2>
